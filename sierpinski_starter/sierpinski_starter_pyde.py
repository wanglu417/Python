...
def sierpinksi(bottomLeft, bottomRight, top, recursionDepth):
    # Fill this in
    if recursionDepth==0 or recursionDepth==1:
        return 0
    else:
        fill(0)
        triangle(bottomLeft.midPoint(bottomRight).getX(),bottomLeft.midPoint(bottomRight).getY(),bottomRight.midPoint(top).getX(),bottomRight.midPoint(top).getY(),top.midPoint(bottomLeft).getX(),top.midPoint(bottomLeft).getY())
        recursionDepth-=1
        sierpinksi(bottomLeft,bottomLeft.midPoint(bottomRight),top.midPoint(bottomLeft),recursionDepth)
        sierpinksi(bottomLeft.midPoint(bottomRight),bottomRight,bottomRight.midPoint(top),recursionDepth)
        sierpinksi(top.midPoint(bottomLeft),bottomRight.midPoint(top),top,recursionDepth)
