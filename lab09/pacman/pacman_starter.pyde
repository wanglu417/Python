WIDTH = 500
HEIGHT = 500
PACMAN_HEIGHT = 100
PACMAN_WIDTH = 100
SPEED = 3
x = WIDTH/2
y = HEIGHT/2
x_add = 0
y_add = 0
MOUTH_DOWN_BEGIN_ANGLE = 90
MOUTH_DOWN_END_ANGLE = 450
MOUTH_UP_BEGIN_ANGLE = -90
MOUTH_UP_END_ANGLE = 270
MOUTH_LEFT_BEGIN_ANGLE = 180
MOUTH_LEFT_END_ANGLE = 540
MOUTH_RIGHT_BEGIN_ANGLE = 0
MOUTH_RIGHT_END_ANGLE = 360
mouth_speed = 5
MAX_MOUTH_ANGLE = 45
MIN_MOUTH_ANGLE = 0
mouth_angle = 0
open_close = 0
move_begin = 0
move_end = 360


def setup():
    size(WIDTH, HEIGHT)
    colorMode(RGB, 1)
    fill(1.0, 1.0, 0.0)
    noStroke()


def draw():
    global x, y
    global mouth_angle
    background(0)
    mouth_angle = mouth_angle + open_close
    x = x + x_add
    y = y + y_add

    # The following cases deal with when PacMan
    # is near the edge of the screen

    # If PacMan moves completely behond the right edge
    if (x > WIDTH + (PACMAN_WIDTH/2)):
        # Reset the x value to the left side
        x = PACMAN_WIDTH/2
    # If PacMan is overlapping the right edge
    elif (x > WIDTH - (PACMAN_WIDTH/2)):
        # draw a second PacMan on the left side, also
        # overlapping
        pacman(x - WIDTH, y)

    # If PacMan moves past the bottom edge,
    # redraw at the top
    if (y > HEIGHT + (PACMAN_HEIGHT/2)):
        y = PACMAN_HEIGHT/2
    elif (y > HEIGHT - (PACMAN_HEIGHT/2)):
        pacman(x, y - HEIGHT)

    # If PacMan moves past the left edge,
    # redraw at the right
    if (x < -(PACMAN_WIDTH/2)):
        x = WIDTH - (PACMAN_WIDTH/2)
    elif (x < PACMAN_WIDTH/2):
        pacman(x + WIDTH, y)

    # If PacMan moves past the top, redraw at bottom
    if (y < -(PACMAN_HEIGHT/2)):
        y = HEIGHT - (PACMAN_HEIGHT/2)
    elif (y < PACMAN_HEIGHT/2):
        pacman(x, y + HEIGHT)

    # Always draw PacMan at his real current location.
    pacman(x, y)


def pacman(x, y):
    """Draw PacMan to the screen"""
    global mouth_angle
    global open_close
    global mouth_speed
    global move_begin
    global move_end
    # Use global variables as necessary
    arc(x, y, PACMAN_WIDTH, PACMAN_HEIGHT,
        radians(move_begin+mouth_angle),
        radians(move_end-mouth_angle))
    if mouth_angle >= MAX_MOUTH_ANGLE:
        open_close = -(mouth_speed)
    if mouth_angle <= MIN_MOUTH_ANGLE:
        open_close = mouth_speed
    mouth_angle += open_close


def keyPressed():
    """Handles keyboard input for PacMan"""
    global move_begin, move_end
    global x_add, y_add  # Must be declared as global
    if (key == CODED):
        if (keyCode == DOWN):
            x_add = 0
            y_add = SPEED
            move_begin = MOUTH_DOWN_BEGIN_ANGLE
            move_end = MOUTH_DOWN_END_ANGLE
        elif (keyCode == UP):
            x_add = 0
            y_add = -(SPEED)
            move_begin = MOUTH_UP_BEGIN_ANGLE
            move_end = MOUTH_UP_END_ANGLE
        elif (keyCode == LEFT):
            x_add = -(SPEED)
            y_add = 0
            move_begin = MOUTH_LEFT_BEGIN_ANGLE
            move_end = MOUTH_LEFT_END_ANGLE
        elif (keyCode == RIGHT):
            x_add = SPEED
            y_add = 0
            move_begin = MOUTH_RIGHT_BEGIN_ANGLE
            move_end = MOUTH_RIGHT_END_ANGLE
