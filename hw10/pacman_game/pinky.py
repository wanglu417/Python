...
class Pinky(GameCharacter):
    ...
    def update(self):
        ...
        # END CODE CHANGES
        if abs(
            ((self.x+1-self.pacman.x)**2+(self.y-self.pacman.y)**2)**0.5
        ) < abs(((self.x-self.pacman.x)**2 + (self.y - self.pacman.y)**2)**0.5
                ):
            if on_horz:
            ...
            ((self.x-self.pacman.x)**2+(self.y+1-self.pacman.y)**2)**0.5
        ) < abs(((self.x-self.pacman.x)**2 + (self.y - self.pacman.y)**2)**0.5
                ):
            if on_vert:
