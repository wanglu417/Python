...
class AIMove():
            ...
            for j in range(BOARDSIZE):
                if board.exist_tiles(i, j):
                    ...
                    for k in self.direction:
                        if i+k[
                            0] >= 0 and i+k[0] < BOARDSIZE and j+k[
                                1] >= 0 and j+k[1] < BOARDSIZE and board.board[
                                    i+k[0]][j+k[1]] == player:
                            for l in range(NEXT_NEXT, BOARDSIZE):
                                if i+l*k[
                                    0] >= 0 and i+l*k[0] < BOARDSIZE and j+l*k[
                                        1] >= 0 and j+l*k[1] < BOARDSIZE:
                                    if board.board[
                                        i+l*k[0]][j+l*k[1]
                                                  ] == player:
                                        continue
    ...
    def computer_move(self, board):
        '''Define AI's move'''
        # If the square is occupied, do nothing
        # and repeat AI_move
        if board.exist_tiles(board.x, board.y):
            pass
        # If the square is not occupied determine
        # whether this is a legal move
        else:
            for i in self.direction:
                # a list that stores positions of squares that
                # possibly lie between two WHITE tiles
                self.temp = list(self.flip)
                if board.x+i[
                    0] >= 0 and board.x+i[0] <= BOARDSIZE - 1 and board.y+i[
                        1] >= 0 and board.y+i[
                            1] <= BOARDSIZE - 1 and board.board[
                                board.x+i[0]][board.y+i[1]
                                              ] == BLACK:
                    ...
                    for j in range(NEXT_NEXT, BOARDSIZE):
                        if board.x+j*i[
                            0] >= 0 and board.x+j*i[
                                ...
                                1] >= 0 and board.y+j*i[1] <= BOARDSIZE - 1:
                            if board.board[
            ...
            # place a WHITE tile, switch to user
            if self.legal:
                delay(1000)
                board.add_tile(board.x, board.y, BLACK)
                for i in self.flip:
                    board.add_tile(i[0], i[1], BLACK)
                board.count += 1
                board.flag = WHITE
                print("User's turn.")
